// Package format provides AST pretty-printing and serialization back to
// expression strings. It supports both compact and indented output modes.
package format
...
// Sprint returns the expression as a string (round-trip from parsed AST).
func Sprint(node parser.Node) string {
	return sprintNode(node, false)
}

// SprintParenthesized returns the expression with full parenthesization.
...
// Indent returns a tree-style representation of the AST for debugging.
...
// RPNString returns the expression in Reverse Polish Notation.
