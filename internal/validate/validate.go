// Package validate performs static analysis on parsed expressions.
// It detects undefined variables, validates function arity, and checks
// for potential runtime errors without actually evaluating the expression.
package validate
...
// Result holds validation outcomes.
type Result struct {
	Issues []Issue
}
...
// FuncSpec describes a built-in function's expected arity.
type FuncSpec struct {
	MinArgs int
	MaxArgs int // -1 = variadic
}

// DefaultFuncs returns the arity specs for all built-in functions.
...
// Validate checks an expression AST for static errors.
// knownVars lists variables that will be available at runtime.
// funcs lists known function signatures.
...
// UndefinedVars returns variable names used in the expression that are not in knownVars.
...
// UsedVars returns all variable names referenced in the expression.
...
// Complexity estimates the computational complexity of an expression
