// Package vm implements a stack-based virtual machine that executes compiled
// bytecode programs. It provides faster repeated evaluation compared to
// tree-walking interpretation when the same expression is evaluated many times
...
// VM is the stack-based virtual machine.
type VM struct {
	stack []float64
	funcs map[string]BuiltinFunc
}

// BuiltinFunc is the signature for VM-level built-in functions.
type BuiltinFunc func(args []float64) (float64, error)

// New creates a VM with default built-in functions.
func New() *VM {
	vm := &VM{
...
// RegisterFunc adds a custom function to the VM.
func (vm *VM) RegisterFunc(name string, fn BuiltinFunc) {
	vm.funcs[name] = fn
}

// RegisterDefaults registers all standard math functions.
...
// StackSize returns the current stack depth (for debugging).
